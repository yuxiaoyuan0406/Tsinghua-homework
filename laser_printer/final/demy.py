...
flaskapp = Flask(__name__)#app是Flask的实例，它接收包或者模块的名字作为参数，但一般都是传递__name__让函数通过传入这个名字确定程序的根目录，以便获得静态文件和模板文件的目录
...
client.connect("192.168.12.1", 1883, 60)                         #连接树莓派

client.subscribe("/values/Warehouse/#", 1)                     #仓库订阅多层消息
client.loop_start()
flaskapp.run(host="0.0.0.0")                            #执行app.run就可以启动服务了。默认Flask只监听虚拟机的本地127.0.0.1这个地址，端口为5000
